          ...
          <>
            ...
            {active ? (
              <Controls
                ...
                style={style}
                onStyleChange={onStyleChange}
                onRemove={(i) => onRemove(i, 'controls')}
...
function DraggableTrinket({ index, label, trinket, style }: { index: number; label: string; trinket?: Trinket; style?: StickerStyle }) {
  ...
  const filter = `drop-shadow(${d1}px ${d1}px 0 var(--sticker-shadow-1)) drop-shadow(${d2}px ${d2}px 0 var(--sticker-shadow-2))`
  return (
    <motion.div
      ref={setNodeRef}
      {...attributes}
    ...
    >
      <div className="sticker" style={{ transform: `scale(${scale}) rotate(${rotate}deg)`, filter }}>
        {trinket?.icon ? (
          ...
          <span className="sticker-inner w-11 h-11 sm:w-14 sm:h-14 text-sm sm:text-base font-black">{trinket?.name?.[0] ?? label[0]}</span>
        )}
      </div>
    </motion.div>
  )
...
function Controls({ index, style, onStyleChange, onRemove, onClose }: {
  ...
  return (
    <motion.div
      ...
      className="hidden sm:flex absolute -top-2 -right-2 z-10 paper p-2 gap-1"
...
function CtrlButton({ children, onClick, label }: { children: any; onClick: () => void; label: string }) {
  return (
    <button
      className="ctrl-btn rounded-md w-7 h-7 text-xs"
      title={label}
      aria-label={label}
      onClick={onClick}
    >
...
function ControlsMobile({ index, style, onStyleChange, onRemove, onClose }: {
  ...
  return (
    <div className="sm:hidden fixed left-0 right-0 bottom-0 z-50 px-2 pb-[calc(env(safe-area-inset-bottom,0)+8px)]" role="dialog" aria-label="Sticker controls" onClick={stop} onMouseDown={stop}>
      <div className="mx-auto max-w-[520px] paper rounded-t-xl rounded-b-none border-b-0 p-2 mobile-controls">
        <div className="flex items-center justify-between gap-1">
          ...
          <button className="chip chip--danger w-10 h-10 p-0" aria-label="Delete" onClick={() => onRemove(index)}>✕</button>
